...
#Clase encargada de la creación de la ventana de inicio de sesión, la cual se encarga de recibir las credenciales ya existentes para una posterior validación y acceso al sistema.
    ...
    #Metodo encargado de verificar si los campos ingresados son validos y en caso de serlo, realiza la solicitud de inicio de sesion.
    def continuar(self):
        ...
        if check[0]:
            ...
            try:
                ...
                self.resultado.config(text=respuesta)
                return      
        self.resultado.config(text=check[1])

    #Metodo encargado de cerrar la ventana actual y regresar al usuario a la ventana anterior en caso de que desee cancelar el inicio de sesión.
    def regresar(self):
        self.destroy()
        VBienvenida()
